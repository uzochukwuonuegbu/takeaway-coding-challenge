...
import { joinGameSchema, makeMoveSchema, startGameSchema } from "./validators/game.validator";
    ...
    public joinGame(): ExpressRouteFunc {
        return async (req: Request, res: Response, next: NextFunction) => {
            try {
                ...
                if (error) {
                    const errorMessage = error.details[0].message;
                    logger.errorLog('Unable to join game', {error: errorMessage})
                    res.status(400).json({ message: errorMessage });
                }
    ...
    public makeMove(): ExpressRouteFunc {
        return async (req: Request, res: Response, next: NextFunction) => {
            try {
                ...
                if (error) {
                    const errorMessage = error.details[0].message;
                    logger.errorLog('Unable to make move', {error: errorMessage})
                    res.status(400).json({ message: errorMessage });
                }
